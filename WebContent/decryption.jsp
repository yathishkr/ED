<%@page import="com.aklc.ed.model.DecryptionHistory"%>
<%@page import="com.aklc.ed.dao.DecryptionHistoryDAO"%>
...
<body>
	...
	<section id="service">
		<div class="container">
			...
			<h2>Decyrption Operation</h2>
			<hr/>
			<div class="service_area">
				<div class="row">
					
					<h3>Decryption of Simple Text</h3>
					<hr/>
					...
					<form action='decrypt'>
						...
						<label>Enter the Encrpted text</label>
						<input name='text' type=text class='form-control' placeholder='Encrypted Text' />
						<br/>
						<input type=submit value='Decrypt' class='btn btn-primary'/>
					</form>
					<hr/>
					<h3>Decryption of Text File</h3>
					<hr/>
					<form action='decrypt?req_type=textfile' method="post" enctype="multipart/form-data">
					...
					<hr/>
					<h3>Decryption History</h3>
					<hr/>				
					
					<% 
						DecryptionHistoryDAO ehDao = new DecryptionHistoryDAO();
						User uModel = (User) session.getAttribute("model");	
						try
						{
							List<DecryptionHistory> result = ehDao.read(uModel.getEmail());
					...
					<table class='table'>
						<tr>
							...
							<th>Entry Time</th>							
								...
								for (DecryptionHistory eh: result)
								{
									%>
						<tr>
							<td> <%=eh.getEmail() %></td>
							<td> <%=eh.getDecryption_type()%></td>
							...
							{
								%>
									<h4>No Decryption History Found </h4>
								<%
							}
						} 
						catch (Exception e)
